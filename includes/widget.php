...
class my_slider_widget extends WP_Widget {
  ...
  public function form($instance) {
    if ( isset( $instance[ 'title' ] ) ) {
      ...
      $mediumImageStyle = $instance[ 'medium_image_style' ];
      $smallImageStyle = $instance[ 'small_image_style' ];
    } else {
    ...
    <p>
    <label for="'.$this->get_field_id( 'medium_image_style' ).'">'.__( 'Image Style (For Medium Size Screens):', 'my-slider' ).'</label>';
    echo '<select class="widefat" id="'.$this->get_field_id( 'medium_image_style' ).'" name="'.$this->get_field_name( 'medium_image_style' ).'">';
    echo '<option value="">'.__( 'None', 'my-slider' ).'</option>';
    foreach ($imageStyles as $key => $value) {
      echo '<option value="'.$key.'"';
      if($key == esc_attr($mediumImageStyle)) echo ' selected';
      echo '>'.$key.'</option>';
    }
    echo '</select>
    </p>
    <p>
    <label for="'.$this->get_field_id( 'small_image_style' ).'">'.__( 'Image Style (For Small Size Screens):', 'my-slider' ).'</label>';
    echo '<select class="widefat" id="'.$this->get_field_id( 'small_image_style' ).'" name="'.$this->get_field_name( 'small_image_style' ).'">';
    echo '<option value="">'.__( 'None', 'my-slider' ).'</option>';
    foreach ($imageStyles as $key => $value) {
      echo '<option value="'.$key.'"';
      if($key == esc_attr($smallImageStyle)) echo ' selected';
      echo '>'.$key.'</option>';
    }
    echo '</select>
    </p>';
  ...
  public function update($new_instance, $old_instance) {
    ...
    $instance['medium_image_style'] = ( ! empty( $new_instance['medium_image_style'] ) ) ? strip_tags( $new_instance['medium_image_style'] ) : '';
    $instance['small_image_style'] = ( ! empty( $new_instance['small_image_style'] ) ) ? strip_tags( $new_instance['small_image_style'] ) : '';
