...
/**
 * Get posts of slider
 * bullets='true' nav-buttons='true' timer='5000' animation='fade' number='-1' image-style='large' pause-on-hover='yes' image-style-medium='medium' image-style-small='thumbnail'
 */
function my_slider_get_slider($bullets, $navButtons, $timer, $animation, $number, $imageStyle, $pause, $imageStyleMedium, $imageStyleSmall) {
  $args = array(
    'numberposts' => $number,
    'post_type'   => 'slide'
  );
  ...
  if($mySlider) {
    ...
    foreach ($mySlider as $slide) :
      ...
      if($imageStyleSmall && !$imageStyleMedium) {
        $slideImageSmall = get_the_post_thumbnail_url($slideID, $imageStyleSmall);
        $output .= '<img class="orbit-image show-for-small-only" src="'.$slideImageSmall.'" alt="'.$slideTitle.'">';
        $output .= '<img class="orbit-image show-for-medium" src="'.$slideImage.'" alt="'.$slideTitle.'">';
  	  } else if(!$imageStyleSmall && $imageStyleMedium) {
        $slideImageMedium = get_the_post_thumbnail_url($slideID, $imageStyleMedium);
        $output .= '<img class="orbit-image show-for-medium-only" src="'.$slideImageSmall.'" alt="'.$slideTitle.'">';
        $output .= '<img class="orbit-image show-for-small-only" src="'.$slideImage.'" alt="'.$slideTitle.'">';
        $output .= '<img class="orbit-image show-for-large-only" src="'.$slideImage.'" alt="'.$slideTitle.'">';
  	  } else if($imageStyleSmall && $imageStyleMedium) {
        $slideImageSmall = get_the_post_thumbnail_url($slideID, $imageStyleSmall);
        $slideImageMedium = get_the_post_thumbnail_url($slideID, $imageStyleMedium);
        $output .= '<img class="orbit-image show-for-small-only" src="'.$slideImageSmall.'" alt="'.$slideTitle.'">';
        $output .= '<img class="orbit-image show-for-medium-only" src="'.$slideImageMedium.'" alt="'.$slideTitle.'">';
        $output .= '<img class="orbit-image show-for-large-only" src="'.$slideImage.'" alt="'.$slideTitle.'">';
  	  } else {
